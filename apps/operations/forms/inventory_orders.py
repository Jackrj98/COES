...
class InventoryOrderBaseForm(forms.ModelForm):
    class Meta:
...
class InboundOrderForm(InventoryOrderBaseForm):
    class Meta(InventoryOrderBaseForm.Meta):
        model = InboundOrder
        fields = InventoryOrderBaseForm.Meta.fields + [
            "supplier",
            "scheduled_date",
            "received_date",
        ]
...
class OutboundOrderForm(InventoryOrderBaseForm):
    class Meta(InventoryOrderBaseForm.Meta):
        model = OutboundOrder
        widgets = {
            "motive": forms.TextInput(
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["order_type"].initial = InventoryOrder.OrderType.OUTBOUND
        self.fields["order_type"].widget = forms.HiddenInput()


class ReplenishmentOrderForm(InventoryOrderBaseForm):
    class Meta(InventoryOrderBaseForm.Meta):
        model = ReplenishmentOrder
        widgets = {
            "motive": forms.TextInput(
                attrs={"class": "form-control datalist-input", "list": "Replenishment-list"}
            ),
        }
...
class OrderDetailForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper = FormHelper()
        self.helper.form_tag = False
        self._apply_logic_by_type()
        for field in self.fields.values():
            field.label = ""

    def _apply_logic_by_type(self):
        fields_to_manage = [
            "supply",
            "batch",
            "expiry_date",
        ...
        logic = {
            ...
            1: ["supply", "quantity_requested", "observations"],
            2: ["supply", "quantity_requested", "observations"],
        }

        active_fields = logic.get(self.order_type, [])
    ...
    def clean(self):
        cleaned_data = super().clean()
        if self.order_type == InventoryOrder.OrderType.INBOUND:
            if not cleaned_data.get("expiry_date"):
                self.add_error("expiry_date", _("Expiry date is required for inbound orders."))
            if not cleaned_data.get("batch"):
                self.add_error("batch", _("Batch is required for inbound orders."))
        return cleaned_data
