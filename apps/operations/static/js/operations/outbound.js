...
function handleSupplySelection(selectElement, supplyData) {
    ...
    if (supplyData.stock !== undefined) {
        ...
        if (quantityInput) {
            // Configuración estricta para salida: limitar al stock disponible
            ...
            quantityInput.oninput = function () {
                ...
                // Forzar límite si el usuario intenta exceder el stock
                if (qty > stock) {
                    qty = stock;
                    this.value = stock;
                }

                // Cálculo exclusivo de salida (Resta)
                const nuevoStock = stock - qty;

                if (newStockSpan) {
                    newStockSpan.textContent = nuevoStock;
                    // Visualización: Si llega a 0, verde (agotado); si es positivo, naranja
                    newStockSpan.style.color = (nuevoStock === 0) ? 'green' : 'orange';
                }
            };
        }
    }
    ...
    const event = new CustomEvent('supplySelected', {
        detail: {supply: supplyData, row: row, type: 1} // Tipo 1 = Salida
    });
