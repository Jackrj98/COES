...
class TestRegisterBatch:
    ...
    def test_register_duplicate_batch_number_raises_error(self, service, valid_batch_payload):
        ...
        # Segundo batch con mismo número
    ...
    def test_register_with_expired_date(self, service, valid_batch_payload):
        ...
        assert batch.status == Batch.BatchStatus.EXPIRED.value
    ...
    def test_update_to_zero_quantity_marks_depleted(self, service, existing_batch):
        ...
        updated_batch = service.update_batch(existing_batch, update_dto)

        assert updated_batch.current_quantity == 0
        assert updated_batch.status == Batch.BatchStatus.DEPLETED.value
...
class TestUpdateBatch:
    ...
    def test_update_partial_fields(self, service, valid_batch_payload):
        batch = service.register_batch(valid_batch_payload)
        update_data = batch
        update_data.batch_number = "BATCH-PARTIAL"
        update_data.current_quantity = 75
        update_data.unit_cost = float("55.00")

        updated = service.update_batch(batch, update_data)

        assert updated.batch_number == "BATCH-PARTIAL"
        assert updated.current_quantity == 75
        assert updated.unit_cost == float("55.00")
...
class TestUpdateBatchStock:
    ...
    def test_update_stock_to_zero_marks_depleted(self, service, existing_batch):
        ...
        assert updated.status == Batch.BatchStatus.DEPLETED.value
...
class TestRetrieveByExpiryDate:
    ...
    def test_retrieve_by_expiry_date_only_active(self, service, valid_batch_payload, supply):
        b1 = valid_batch_payload
        b1.batch_number = "BATCH-ACTIVE"
        service.register_batch(b1)

        n_valid_batch_payload = valid_batch_payload
        n_valid_batch_payload.batch_number = "BATCH-INACTIVE"
        n_valid_batch_payload.is_active = False

        service.register_batch(n_valid_batch_payload)

        n2_batch_payload = valid_batch_payload
        n2_batch_payload.batch_number = "BATCH-EMPTY"
        n2_batch_payload.current_quantity = 0
        service.register_batch(n2_batch_payload)
